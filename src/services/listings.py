...
from src.repositories.postgres.comment import add_comment
...
async def auto_create_listings(session: AsyncSession) -> dict[str, True]:
    ...
    for i in range(1, 11):
        ...
        await add_listing(session, Listing(title=f"title{i}", created_at=datetime.now(), description=f"description{i}",
                                           price=i * 1000,
    ...
    for i in range(1, 11):
        random_user_id = randint(1, 10)
        await add_comment(session, Comment(description=f"description{i}", user_id=random_user_id,
                                           listing_id=choice(listings_id)))
        await add_favorite(session, Favorite(user_id=random_user_id, listing_id=choice(listings_id)))
    return {"ok": True}
